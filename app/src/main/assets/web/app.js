...
const state={
  ...
  nativeRemoteMessage:nativeAndroid?'Spotify conectado · segundo plano listo':'',nativeTrack:null
};
...
function playbackLabel(){const d=selectedExternal()||activeDevice();return d?.name?String(d.name).slice(0,18):(nativeAndroid?'SPOTIFY':'2º PLANO')}
...
function deviceRows(){
  ...
  if(!ext.length)return `<div class="no-device"><b>Spotify está conectado, falta activar el reproductor del celular</b><span>Abre Spotify oficial una vez. Cuando vuelvas, Musikplay detectará el teléfono automáticamente y quedará listo para reproducir en segundo plano.</span><button class="primary full device-open" data-action="open-spotify-native">Abrir Spotify oficial</button></div>`;
  return `<div class="device-list">${ext.map(d=>`<button class="device-row ${d.id===state.selectedDeviceId?'selected':''}" data-action="select-device" data-device="${esc(d.id)}"><span class="device-ico">${deviceIcon(d.type)}</span><span><b>${esc(d.name)}</b><small>${esc(d.type||'Spotify Connect')}${d.is_active?' · Activo':''}</small></span><i>${d.id===state.selectedDeviceId?'✓':d.is_active?'●':'○'}</i></button>`).join('')}</div>`;
}
...
${state.connected?`<div class="settings-card hifi-card"><span class="eyebrow">REPRODUCCIÓN</span><h3 class="card-title">${nativeAndroid?'Spotify oficial · segundo plano':'Motor de reproducción'}</h3>${nativeAndroid?`<p class="engine-copy">Musikplay mantiene la interfaz y controla la reproducción mediante Spotify Connect. El audio queda en Spotify oficial para que siga sonando al bloquear la pantalla o cambiar de app.</p><div class="device-head"><span>Dispositivo de audio</span><button data-action="refresh-devices">↻ Actualizar</button></div>${deviceRows()}`:`<div class="mode-switch"><button class="${state.playbackMode==='background'?'on':''}" data-action="mode" data-mode="background"><b>◉ Spotify oficial</b><span>Segundo plano</span></button><button class="${state.playbackMode==='web'?'on':''}" data-action="mode" data-mode="web"><b>♫ Musikplay Web</b><span>Respaldo</span></button></div><div class="mode-explain">${state.playbackMode==='background'?`<b>Segundo plano + máxima calidad disponible</b><p>El audio queda en Spotify oficial/Spotify Connect. Puedes bloquear el celular o cambiar de app sin cortar la música.</p><div class="device-head"><span>Dispositivo de audio</span><button data-action="refresh-devices">↻ Actualizar</button></div>${deviceRows()}`:`<b>Reproductor dentro de Musikplay</b><p>Usa Spotify Web Playback SDK y Media Session.</p><div class="web-status"><span class="connection-dot ${state.sdkReady?'on':''}"></span>${state.sdkReady?'Musikplay Web listo':'Activando reproductor web…'}</div>`}</div>`}</div>`:''}
<div class="settings-card"><div class="setting-line"><div class="setting-icon">◉</div><div><h3>Estado del reproductor</h3><p>${state.connected?(selectedExternal()?`Listo en ${esc(selectedExternal().name)}`:'Spotify conectado · abre Spotify oficial una vez para activar este celular'):'Disponible al conectar Spotify'}</p></div><span class="quality ${state.connected&&selectedExternal()?'on':''}">${nativeAndroid?'BG':'WEB'}</span></div><p class="muted quality-note">Musikplay no recomprime el audio. La calidad final depende de la configuración de Spotify y del dispositivo de salida.</p>${state.playerError?`<p class="error-copy">${esc(state.playerError)}</p>`:''}</div>
...
${state.installEvent?`<div class="settings-card"><div class="setting-line"><div class="setting-icon">▣</div><div><h3>Instalar Musikplay</h3><p>Úsala como una app desde la pantalla de inicio.</p></div><button data-action="install">Instalar</button></div></div>`:''}<div class="about"><span>Musikplay Android V2.2</span><span>Spotify Connect · segundo plano</span></div></section>`}

function miniPlayer(){const c=current();if(!state.connected&&!c)return '';if(c)return `<div class="mini-player visible"><button class="mini-art-btn" data-action="open-player">${art(c,2)}</button><div class="mini-copy" data-action="open-player"><strong>${esc(c.name)}</strong><span>${esc(artists(c))}</span><small>◉ ${esc(playbackLabel())}</small></div><button class="mini-skip" data-action="previous" aria-label="Anterior">⏮</button><button class="mini-toggle" data-action="toggle" aria-label="Reproducir o pausar">${isPaused()?'▶':'Ⅱ'}</button><button class="mini-skip" data-action="next" aria-label="Siguiente">⏭</button></div>`;return `<div class="mini-player waiting"><div class="waiting-disc">♫</div><div class="mini-copy"><strong>Spotify conectado</strong><span>${selectedExternal()?`Listo en ${esc(selectedExternal().name)}`:'Abre Spotify oficial una vez para activar este celular'}</span></div>${nativeAndroid&&!selectedExternal()?`<button class="mini-connect" data-action="open-spotify-native">Abrir</button>`:''}</div>`}
...
async function syncPlayback(full=false){
  ...
  try{
    const pb=await getPlaybackState();
    state.remotePlayback=pb||null;
    if(full||Date.now()-state.lastDeviceSync>10000)await refreshDevices(false);
    if(state.showPlayer){try{const q=await getQueue();state.queue=q?.queue||[]}catch{}}
    render();
  }catch(e){if(e.message!=='NOT_CONNECTED')state.playerError=e.message}
}
async function requireTarget(uri='',pendingAction='playTrack'){
  if(state.playbackMode==='web'&&!nativeAndroid){
    await player?.activateElement?.();
    if(!state.deviceId)throw new Error('Musikplay Web todavía se está activando.');
    return state.deviceId;
  }
  await refreshDevices(false);
  const d=selectedExternal();
  if(d){
    state.selectedDeviceId=d.id;
    localStorage.setItem('musikplay_device_id',d.id);
    return d.id;
  }
  if(nativeAndroid){
    try{
      if(uri) sessionStorage.setItem('musikplay_pending_play',JSON.stringify({uri,action:pendingAction}));
      if(uri) window.AndroidMusikplay?.openSpotifyContent?.(uri);
      else window.AndroidMusikplay?.openSpotify?.();
    }catch{}
    throw new Error(uri?'Abrí Spotify para activar este celular. Cuando vuelvas a Musikplay intentaré reproducir automáticamente.':'Abrí Spotify para activar este celular. Vuelve a Musikplay cuando esté abierto.');
  }
  throw new Error('Abre Spotify oficial en tu celular, reproduce o pausa una canción una vez y vuelve a Musikplay.');
}
async function playUri(uri){
  const t=findTrackByUri(uri);
  if(t?.demo){state.showSetup=true;notify('Conecta Spotify para reproducir música real.');return}
  try{
    const id=await requireTarget(uri,'playTrack');
    if(t) state.remotePlayback={is_playing:true,item:t,device:state.devices.find(d=>d.id===id)||{id,name:'Spotify',type:'Smartphone'}};
    render();
    await controlPlayer('playTrack',id,{uri});
    setTimeout(()=>syncPlayback(true),700);
...
async function playContext(uri){
  ...
  try{
    const id=await requireTarget(uri,'playContext');
    await controlPlayer('playContext',id,{uri});
    setTimeout(()=>syncPlayback(true),700);
  }catch(e){notify(e.message)}
}
async function transport(action){
  try{
    const id=await requireTarget(current()?.uri||'');
    if(action==='toggle')await controlPlayer(isPaused()?'resume':'pause',id);
    else await controlPlayer(action,id);
    setTimeout(()=>syncPlayback(false),500);
  }catch(e){notify(e.message)}
}
...
async function resumeAfterSpotify(){
  if(!state.connected)return;
  try{
    await refreshDevices(false);
    const raw=sessionStorage.getItem('musikplay_pending_play');
    const pending=raw?JSON.parse(raw):null;
    const d=selectedExternal();
    if(pending?.uri&&d){
      sessionStorage.removeItem('musikplay_pending_play');
      state.selectedDeviceId=d.id;
      localStorage.setItem('musikplay_device_id',d.id);
      await controlPlayer(pending.action==='playContext'?'playContext':'playTrack',d.id,{uri:pending.uri});
      notify(`Listo · reproduciendo en ${d.name}`);
      setTimeout(()=>syncPlayback(true),650);
      return;
    }
    await syncPlayback(true);
  }catch(e){state.playerError=e.message;render()}
}
